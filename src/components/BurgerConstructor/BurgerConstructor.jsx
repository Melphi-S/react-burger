...
const BurgerConstructor = ({ingredients, selectedIngredientIds}) => {
  ...
  const bun = ingredients.find(ingredient => ingredient._id === ingredientIds.bunId);
  const selectedIngredients = ingredientIds.toppingIds.map(id => ingredients.find(ingredient => ingredient._id === id) );
  const totalPrice = bun.price * 2 + selectedIngredients.reduce((sum, ingredient) => sum + ingredient.price, 0);
